...
`}function h(){let e=o.ptQuery.trim().toLowerCase(),t=o.points.map((e,t)=>({p:e,i:t})).filter(({p:t})=>!e||t.name.toLowerCase().includes(e)||(t.city||``).toLowerCase().includes(e)||(t.cluster||``).toLowerCase().includes(e)),n=[...new Set(o.points.map(e=>e.city))],r=[...new Set(o.points.map(e=>e.cluster))],i=`<div class="ae-hint">从左侧选择一个点位，或点「新增点位」</div>`;if(o.editIdx>=0){let e=o.points[o.editIdx];i=`
      <div class="ae-form">
        ...
        <div class="ae-row">
          <label>置信度
            <select id="pt-conf">
              <option value="100" ${(e.confidence??100)===100?`selected`:``}>✓ 已核实（100）</option>
              <option value="75" ${e.confidence===75?`selected`:``}>⚠ 中置信（75）</option>
              <option value="0" ${e.confidence===0?`selected`:``}>✗ 未通过（0）</option>
            </select></label>
          <label>标签<input id="pt-tags" value="${a((e.tags||[]).join(`, `))}" placeholder="逗号分隔，如 免费, 周一闭馆">
            <span class="ae-tip">用于筛选 chips，逗号分隔多个</span></label>
        </div>
        <div class="ae-row">
    ...
    <div class="ae-form" style="max-width:520px">
      ...
      <label>备案号（ICP）<input id="st-icp" value="${a(o.site.icp||``)}" placeholder="如 沪ICP备XXXXXXXX号，留空则不显示">
        <span class="ae-tip">显示在所有公开页面底部，自动链接到 beian.miit.gov.cn</span></label>
      <div class="ae-btns">
